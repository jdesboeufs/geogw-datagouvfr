...
exports.publishAll = function (req, res) {
    ...
    Dataset
        ...
        .pipe(es.map(function (dataset, done) {
            dataset
                .set('publication.organization', req.organization._id)
                .set('publication.sourceCatalog', req.organization.sourceCatalog)
                .set('publication.status', 'public')
...
exports.syncAll = function (req, res) {
    ...
    Dataset
        ...
        .stream()
        .pipe(es.map(function (dataset, done) {
            dataset.synchronize(function (err) {
                if (err) return done(err);
                count++;
                done(null, null);
            });
