...
module.exports = function (job, jobDone) {
            ...
            geogw.fetchRecordsFromCatalog(organization.sourceCatalog, query, function (err, resp) {
                ...
                async.eachLimit(resp.results, 20, function (dataset, datasetDone) {
                    if (dataset._id in datasetsProcessed) {
                        ...
                        Dataset.findByIdAndUpdate(dataset._id, updateOperations, opts, function (err, datasetUpdated) {
                            if (err) {
                                debug('Unable to upsert matching dataset %s', dataset._id);
    ...
    async.series([fetchOrganization, fetchProducers, cleanMatchingDatasets, fetchAndSaveDatasets], jobDone);
