...
exports.map = function (sourceDataset) {
    ...
    if (sourceDataset.distributions) {
        var processedFeatureTypes = [];

        sourceDataset.distributions.forEach(function (distribution) {
            if (!distribution.available) return;

            if (distribution.type === 'wfs-featureType') {
                var rootUrl = process.env.GEOGW_URL + '/api/services/' + distribution.service + '/feature-types/' + distribution.typeName + '/download';
                ...
                out.resources.push({
                    ...
                    title: simplifiedTypeName + ' (KML / WGS-84)',
                    description: 'Conversion à la volée du jeu de données d\'origine ' + simplifiedTypeName + ' au format KML (WGS-84)',
                    format: 'KML',
                    type: 'api'
                });
                out.resources.push({
                ...
                out.resources.push({
                    ...
                    title: 'Télécharger le package complet (données + documents)',
                    format: 'TAB',
                    type: 'file'
                });
            }
